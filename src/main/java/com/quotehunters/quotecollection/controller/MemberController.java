...
public class MemberController {
    ...
    // [Member-001] 회원가입 업무와 DB 처리를 담당하는 Service 객체다.
    ...
    // [Member-001] View가 받은 ID와 비밀번호를 Service에 전달하고, 로그인 회원 DTO 또는 null을 돌려준다.
    public MemberDTO login(String userId, String userPw) {
        return memberService.login(userId, userPw);
    }

    // [Member-004] 현재 비밀번호가 로그인한 회원의 DB 비밀번호와 같은지 확인한다.
